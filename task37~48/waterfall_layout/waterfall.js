	...
	WaterFall.prototype.addImg = function(addNum){
		...
		xhr.onreadystatechange = function(){
			...
			if(xhr.readyState === 4){
				...
				// 如果图片索引大于图片数量时显示提示文字,否则执行addImg加载图片
				if(this.imgIndex > imgData.length){
					if(!this.noMoreImg){
		...
		function eachAddImg(){
			...
			for(var i=0; i<addImgNum; i+=1){
				...
				img.src = this.directory + imgData[1+this.imgIndex];
	...
	var waterFall = new WaterFall('#container',5,'./image/waterfall_img/');
	waterFall.addImg(20);
